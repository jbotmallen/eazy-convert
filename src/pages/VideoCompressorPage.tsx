...
const MAX_RESOLUTION_OPTIONS: { value: "original" | 360 | 480 | 720 | 1080 | 1440 | 2160; label: string }[] = [
...
export function VideoCompressorPage() {
  ...
  const [customCrf, setCustomCrf] = useState(26);
  const [customMaxHeight, setCustomMaxHeight] = useState<"original" | 360 | 480 | 720 | 1080 | 1440 | 2160>("original");
  ...
  const setPresetById = (presetId: CompressionPresetId) => {
  ...
  const handleCompress = async () => {
    ...
    const compressionOptions = showAdvancedControls ? {
      crf: customCrf,
      maxHeight: customMaxHeight === "original" ? undefined : customMaxHeight,
    } : undefined;
    const parsedOptions = compressionOptionsSchema.safeParse(compressionOptions);
    if (!parsedOptions.success) {
      showErrorToast("Advanced options must use a whole-number CRF from 18 to 35 and a supported max resolution.");
      return;
    }
    ...
    for (const file of queue) {
      ...
      try {
        const result: CompressionResult = await window.api.video.compress(file.fileId, preset, outputFormat, compressionOptions);
        ...
        if (cancelledRef.current) {
          ...
          const msg = getUserErrorMessage(err, "Compression failed.");
          setFiles(prev => prev.map(f => f.id === file.id ? {
            ...f,
            status: "error",
            error: msg,
  ...
  const activePreset = COMPRESSION_PRESETS.find(item => item.id === preset) ?? COMPRESSION_PRESETS[2];
  const compressionOptions = showAdvancedControls ? {
    crf: customCrf,
    maxHeight: customMaxHeight === "original" ? undefined : customMaxHeight,
  } : undefined;
  const advancedValidation = compressionOptionsSchema.safeParse(compressionOptions);
      ...
      <Card className="w-full overflow-hidden border-border bg-card/40 font-sans shadow-2xl backdrop-blur-md">
        <CardHeader className="border-b border-border/50 bg-muted/5 pb-8 text-center">
          ...
          <CardDescription className="mx-auto max-w-md text-base font-medium italic">
            Batch compress MP4, WebM, AVI, MOV, and MKV videos into smaller local files.
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                {COMPRESSION_PRESETS.map((presetOption) => (
                  <button
                    key={presetOption.id}
                    type="button"
                ...
                >
                  <div className="mt-5 grid gap-3 md:grid-cols-[1fr_5rem] md:items-center">
                    <label className="space-y-2">
                      <span className="flex items-center justify-between text-xs font-black uppercase tracking-[0.18em] text-muted-foreground">
                        <FieldLabel as="span" icon={Gauge} className="m-0 tracking-[0.18em]">Custom CRF</FieldLabel>
                      </span>
                      <input
                        type="range"
                        min={18}
                        ...
                        value={customCrf}
                        disabled={isCompressing}
                        onChange={(event) => setCustomCrf(Number(event.target.value))}
                    ...
                    <label className="space-y-2 md:col-span-2">
                      <FieldLabel as="span" icon={Ruler} className="m-0 tracking-[0.18em]">Max Resolution</FieldLabel>
                      <Select
                        value={String(customMaxHeight)}
                        disabled={isCompressing}
                        onValueChange={(value) => {
                          setCustomMaxHeight(value === "original" ? "original" : Number(value) as 360 | 480 | 720 | 1080 | 1440 | 2160);
                        }}
                      >
                        <SelectTrigger className="h-14 text-lg font-bold border-2 uppercase italic bg-transparent">
                          <SelectValue />
                  ...
                  {!advancedValidation.success && (
                    <p className="mt-2 text-xs font-bold text-destructive">
                      Advanced options must use a whole-number CRF from 18 to 35 and a supported max resolution.
                    </p>
                  )}
                </motion.div>
              )}
