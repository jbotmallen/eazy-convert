...
import {
  AUDIO_DENOISE_OUTPUT_FORMATS,
  DENOISE_ENGINES,
  DENOISE_PRESETS,
  RNNOISE_MODELS,
  type AudioDenoiseOutputFormat,
  type DenoiseEngine,
  type DenoisePreset,
...
export function AudioDenoiserPage() {
  ...
  const [outputFormat, setOutputFormat] = useState<AudioDenoiseOutputFormat>("mp3");
    ...
    try {
      const options =
        engine === "rnnoise"
          ? { engine, preset, model }
          : { engine, preset };
      const result = await window.api.audio.denoise(file.id, outputFormat, options);
      if (cancelledRef.current) return;
      setOutputPath(result.outputPath);
      showSuccessToast("Audio denoised successfully.");
    } catch (err) {
      if (cancelledRef.current) return;
  ...
  }, [file, engine, preset, model, outputFormat, setIsProcessing]);
      ...
      <Card className="w-full overflow-hidden border-border bg-card/40 font-sans shadow-2xl backdrop-blur-md">
        <CardHeader className="border-b border-border/50 bg-muted/5 pb-8 text-center">
          ...
          <CardDescription className="mx-auto max-w-md text-base font-medium italic">
            Remove background noise from audio files. RNNoise for speech, FFT for hiss and hum.
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between w-full gap-2">
              ...
              <div className="space-y-2 w-full">
                ...
                <Select
                  value={outputFormat}
                  onValueChange={(v) => setOutputFormat(v as AudioDenoiseOutputFormat)}
                  disabled={isRunning}
                >
                  <SelectTrigger className="h-14 text-lg font-bold border-2 uppercase italic bg-transparent">
                    <SelectValue />
                  ...
                  <SelectContent className="bg-card/90 backdrop-blur-xl border-2">
                    {AUDIO_DENOISE_OUTPUT_FORMATS.map((f) => (
                      <SelectItem
                        key={f}
                        value={f}
                        className="font-bold uppercase italic py-3 cursor-pointer"
                ...
                <p className="ml-1 text-xs font-medium text-muted-foreground">
                  Output format for the denoised audio file.
                </p>
              </div>
            </div>
          </div>
          ...
          {!file ? (
            ...
            >
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="space-y-2">
                  <p className="text-xl font-bold tracking-tight">Click or drop an audio file</p>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="hidden shrink-0 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 sm:block">
                  {getDotExt(file.name).slice(1)} to {outputFormat}
                </span>
